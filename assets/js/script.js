...
//1. Punto - Agregando elementos con Push
traumatologia.push({ hora: '9:00', especialista: 'RENÉ POBLETE', paciente: 'ANA GELLONA', rut: '13123329-7', prevision: 'ISAPRE' },
    ...
    { hora: '10:30', especialista: 'ANTONIO LARENAS', paciente: 'CPABLO LOAYZA', rut: '13453234-1', prevision: 'ISAPRE' },
...
listadoPacientes(radiologia)
listadoPacientes(traumatologia)
listadoPacientes(dental)

function listadoPacientes(array) {
    for (const paciente of array) {
        document.write(`<p>${paciente.paciente}</p>`)
    }
}

//5. Punto
document.write("<h2>Lista pacientes ISAPRE para Dental</h2>")
filtroIsapre(dental, "ISAPRE")

//6. Punto
document.write("<h2>Lista pacientes FONASA para Traumatología</h2>")
filtroIsapre(traumatologia, "FONASA")

//Funcion para filtrar el pto 5 y 6
function filtroIsapre(array, prevision) {
    let filtroPrevision = array.filter(function(array){
        return array.prevision == prevision
    })

    for (let i = 0; i < filtroPrevision.length; i++) {
        document.write(`<p>${filtroPrevision[i].paciente} - ${filtroPrevision[i].prevision}</p>`)
    }
}
document.write("<p>*************************************************</p>")
//Fin ejercicio
